...
function DefectRateChecker() {
    ...
    const fetchDefectRate = async () => {
        ...
        try {
            const url = `https://daelim-semiconductor.duckdns.org:8443/api/data/defectRate?fieldName=${encodeURIComponent(fieldName)}&thresholdPercentage=${encodeURIComponent(thresholdPercentage)}`;
    ...
    const handleIdClick = async (id) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const url = `http://daelim-semiconductor.duckdns.org:8443/api/data/${encodeURIComponent(id)}`;
            const response = await fetch(url, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
