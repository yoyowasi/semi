...
const AdminPage = () => {
    ...
    const fetchUnapprovedUsers = async () => {
        ...
        try {
            const response = await axios.get('http://daelim-semiconductor.duckdns.org:8080/api/user/unapproved-users', {
                headers: {
    ...
    const approveUser = async (user_no) => {
        const token = localStorage.getItem("token"); // JWT 토큰 가져오기
        try {
            await axios.post(`http://daelim-semiconductor.duckdns.org:8080/api/user/approve-user/${user_no}`, {}, {
